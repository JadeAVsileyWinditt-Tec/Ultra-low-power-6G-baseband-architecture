//==============================================================================
// fabric_slice.sv – Reusable hierarchical slice of the baseband fabric
//
// Contains:
//   - A small array of DSP tiles (OFDM FFT capable)
//   - Regional TBCU
//   - Multi-cycle power reduction
//   - NoC address map for each tile
//
// Multiple slices can later be stitched together to reach 2048 tiles.
//==============================================================================
...
module fabric_slice #(
  ...
  // Global workload / FFT stimulus (broadcast for this skeleton)
  ...
  //--------------------------------------------------------------------------
  // DSP tile array + address map
  //--------------------------------------------------------------------------
  genvar t;
  generate
    for (t = 0; t < NUM_TILES; t++) begin : g_tiles
      // Hierarchical address for this tile
      noc_addr_map #(
        .N_TILES   (N_TILES),
        .TILES_X   (TILES_X),
        .TILES_Y   (TILES_Y),
  ...
  //--------------------------------------------------------------------------
  // Observe any FFT completion in the slice
  //--------------------------------------------------------------------------
  always_comb begin
    any_fft_valid = 1'b0;
